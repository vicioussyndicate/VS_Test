    ...
    with open(DRFILENAME, "r") as csvfile:
        ...
        for row in datareader:
            ...
            if count == 1:
                ...
                table_week_L_5 = newTable(dt)
                table_month_L_5 = newTable(dt)
                
                table_week_6_15 = newTable(dt)
                table_month_6_15 = newTable(dt)
                ...
                if he_arch in hsArchetypes:
                    ...
                    if t_delta.days < 7:
                        ...
                        if rank >= 0 and rank <= 5:
                            table_week_L_5['table'][idx_he][idx_op][res] += 1
                            table_week_L_5['frequency'][idx_op] += 1
                        if rank >= 6 and rank <= 15:
                            table_week_6_15['table'][idx_he][idx_op][res] += 1
                            table_week_6_15['frequency'][idx_op] += 1
                    ...
                    if t_delta.days < 30:
                        ...
                        if rank >= 0 and rank <= 5:
                            table_month_L_5['table'][idx_he][idx_op][res] += 1
                            table_month_L_5['frequency'][idx_op] += 1
                        if rank >= 6 and rank <= 15:
                            table_month_6_15['table'][idx_he][idx_op][res] += 1
                            table_month_6_15['frequency'][idx_op] += 1
        ...
        saveTableAsJson(table_week,dt,hsFormat,'lastWeek','ranks_all')
        saveTableAsJson(table_month,dt,hsFormat,'lastMonth','ranks_all')

        saveTableAsJson(table_week_L_5,dt,hsFormat,'lastWeek','ranks_L_5')
        saveTableAsJson(table_month_L_5,dt,hsFormat,'lastMonth','ranks_L_5')

        saveTableAsJson(table_week_6_15,dt,hsFormat,'lastWeek','ranks_6_15')
        saveTableAsJson(table_month_6_15,dt,hsFormat,'lastMonth','ranks_6_15')
                    
            


def main():
    for hsFormat in hsFormats:
        run(hsFormat)
