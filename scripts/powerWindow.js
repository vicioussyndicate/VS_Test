class PowerWindow {
  constructor() {
    ...
    // removed ('All Ranks', 'Rank 1-5' etc.) & until rank 20. text from below overlay text
    this.overlayText = `
            This tab displays the best decks to be played in the respective rank brackets.<br><br>
            <span class='optionBtn'>Tier Lists</span> shows the top 16 decks across specific rank brackets. <br><br>
            <span class='optionBtn'>Suggestions</span> shows the top 5 decks for every single rank<br><br>
    ...
    this.bracketData = {};
    // this.rankBrackets = [ { name: "All Ranks", games: {}, start: 0, end: 15 }, { name: "L", games: {}, start: 0, end: 0 }, { name: "1-4", games: {}, start: 1, end: 4 }, { name: "5-14", games: {}, start: 5, end: 14 },];
    // this.rankBrackets = [ { name: "All Ranks", games: {}, start: 0, end: 50 }, { name: "L", games: {}, start: 0, end: 0 }, { name: "Diamond-1-10", games: {}, start: 1, end: 10 }, { name: "Platinum-1-10", games: {}, start: 11, end: 20 }, { name: "Gold-1-10", games: {}, start: 21, end: 30 }, { name: "Silver&Bronze", games: {}, start: 31, end: 50 },];
    this.rankBrackets = [
      { name: "All ranks", games: {}, start: 0, end: 19 },
      { name: "Legend", games: {}, start: 0, end: 0 },
      { name: "Diamond 1-4", games: {}, start: 1, end: 4 },
      { name: "Diamond 5-10", games: {}, start: 5, end: 10 },
      { name: "Platinum", games: {}, start: 11, end: 15 },
      { name: "Gold Silver Bronze", games: {}, start: 16, end: 19 },
    ];
  ...
  setupUI() {
    for (let i = 0; i < this.optionButtons.length; i++) {
      this.optionButtons[i].addEventListener(
        "click",
    ...
    document.querySelector(
      "#powerWindow .content-header #brackets"
    ).style.display = disp;
  ...
  addData(f, callback) {
    ...
    this.rankData.rankSums[f] =
      app.ui.ladderWindow.data[f][this.t_ladder[f]].rankSums;
    for (let rank of range(0, hsRanks)) {
      for (let bracket of this.rankBrackets) {
        if (bracket.start <= rank && bracket.end >= rank) {
          bracket.games[f] += this.rankData.rankSums[f][rank];
        }
      }
    }
      ...
      for (let rank of range(0, hsRanks)) {
        ...
        for (var bracket of this.rankBrackets) {
          ...
          if (rank > bracket.start && rank <= bracket.end) {
            data[data.length - 1].wr += totWr;
            data[data.length - 1].count += totWr > 0 ? 1 : 0;
          }
          if (rank == bracket.end && data[data.length - 1].count > 0) {
            data[data.length - 1].wr /= data[data.length - 1].count;
          }
    ...
    let sortByWr = function (a, b) {
      return a.wr > b.wr ? -1 : a.wr < b.wr ? 1 : 0;
    };
    for (let rank of range(0, hsRanks)) {
      this.rankData[f][rank].sort(sortByWr);
    }
    for (let bracket of this.rankBrackets) {
      this.bracketData[f][bracket.name].sort(sortByWr);
    }
  ...
  checkLoadData(callback) {
    ...
    if (!app.ui.ladderWindow.data[this.f].fullyLoaded) {
      let callback2 = function () {
        app.ui.powerWindow.checkLoadData(callback);
      };
      if (back) {
        return app.ui.ladderWindow.loadData(this.f, callback2);
      } else {
        return false;
      }
    }
    ...
    if (
      app.ui.ladderWindow.data[this.f].fullyLoaded &&
  ...
  plot() {
    ...
    this.renderOptions();
    if (this.mode == "ranks") {
      this.plotRanks(this.f);
    }
    if (this.mode == "brackets") {
      this.plotBrackets(this.f);
    }
  }
  ...
  plotRanks(f) {
    ...
    let ranks = rankLabels; //range(0, hsRanks);
    //ranks[0] = "L";
  ...
  plotBrackets(f) {
    ...
    let columnTemplate = "";
    for (let b of this.rankBrackets) {
      columnTemplate += "4fr 1fr ";
    }
    ...
    for (let i = 0; i < this.maxElementsPerBracket; i++) {
      for (let bracket of this.rankBrackets) {
        let temp_var = this.bracketData[f][bracket.name];
        if (this.bracketData[f][bracket.name].length <= i) {
          continue;
        }
