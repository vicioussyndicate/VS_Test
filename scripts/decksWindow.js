class DecksWindow {
  constructor(callback) {
    ...
    this.descriptionBox = document.querySelector(
      "#decksWindow .content .descriptionBox"
    );
    this.decksDiv = document.querySelector("#decksWindow .content .decklists");
    this.description = document.querySelector(
      "#decksWindow .content .descriptionBox .description"
    );
  ...
  setupUI() {
    this.dropdownFolders = {
      format: document.querySelector(
        "#decksWindow .content-header #formatFolder .dropdown"
      ),
      class: document.querySelector(
        "#decksWindow .content-header #classFolder .dropdown"
      ),
    };
    ...
    this.compareBtn = document.querySelector(
      "#decksWindow .content-header #compare"
    );
    this.infoBtn.active = false;
    this.compareBtn.active = false;

    this.selection = {};
    this.selection.div = document.querySelector(
      "#decksWindow .selectionWrapper"
    );
    this.selection.buttonWrapper = document.querySelector(
      "#decksWindow .selectionWrapper .buttonWrapper"
    );
  ...
  buttonTrigger(e) {
    ...
    if (e.target.classList.contains("archBtn")) {
      ...
      this.mode = "decklists";
      /*  Meta and Random button removed
  ...
  loadArchetypes(w) {
    // trigger when press class/ random/ meta button

    if (!this.checkLoadData()) {
      return this.checkLoadData((_) => {
        app.ui.decksWindow.loadArchetypes(w);
      });
    }
    ...
    if (w == "Meta" || w == "Random") {
      if (w == "Random") {
        this.archetypes[this.f] = shuffle(this.archetypes[this.f]);
      }
      for (let a of this.archetypes[this.f].slice(0, 5)) {
        if (a.decklists.length > 0) {
          this.sidebar.addArchBtn(a);
        }
      }
    }
  ...
  loadDecklists(w) {
    // trigger when pushed on archetype button, w == Meta, Random, ArchName
  ...
  compare(bool) {
    ...
    for (let idx1 of range(0, numDl)) {
      for (let card of this.decklists[idx1].cards) {
        if (cards.indexOf(card.name) != -1) {
          continue;
        } // card has been classified already
        ...
        if (!only1DeckHas1or2 && !allDecksHave1) {
          someDecksDontHave1or2 = true;
        }

        let classification = "";
        if (allDecksHave1) {
          classification = "core_x1";
        }
        if (allDecksHave2) {
          classification = "core_x2";
        }
        if (someDecksDontHave1or2) {
          classification = "some";
        }
        if (only1DeckHas1or2) {
        ...
        for (let dl of this.decklists) {
          dl.classify(card.name, classification);
        }
      }
    }
  }
  ...
  renderOptions() {
    ...
    if (this.mode == "decklists") {
      document.querySelector("#decksWindow .displayOptions").style.display =
        "flex";
    } else {
      document.querySelector("#decksWindow .displayOptions").style.display =
        "none";
    ...
    document.querySelector("#decksWindow #formatBtn").innerHTML =
      btnIdToText[this.f];
  }
  ...
  checkLoadData(callback) {
    ...
    if (!app.ui.tableWindow.data[this.f].fullyLoaded) {
      let callback2 = function () {
        app.ui.decksWindow.checkLoadData(callback);
      };
      if (back) {
  ...
  loadData(hsFormat, callback) {
    ...
    ref.on("value", reader.bind(this), (e) =>
      console.log("Could not load Deck Data", e)
    );
  }
  ...
  readData(DATA, hsFormat, callback) {
    ...
    let data = DATA.val();
    console.log({ decksData: data });
    let f = hsFormat;

    for (let hsClass of hsClasses) {
      this.data[f][hsClass].archetypes = []; // delete existing
      this.data[f][hsClass].text = data[hsClass].text;
      if (!("archetypes" in data[hsClass])) {
        continue;
      } // sometimes a class has 0 archetypes
      ...
      for (let key in data[hsClass].archetypes) {
        ...
        if (mu_idx >= 0) {
          wr = this.mu[f].wr[mu_idx];
          fr = this.mu[f].fr[mu_idx];
        }
  ...
  deckLink(archName) {
    ...
    if (!this.checkLoadData()) {
      return this.checkLoadData((_) => {
        app.ui.decksWindow.deckLink(archName);
      });
    }
  ...
  loadClass(hsClass) {
    ...
    this.archetypes[this.f].sort(wrSort);
    for (let a of this.archetypes[this.f]) {
      if (a.hsClass == this.hsClass && a.decklists.length > 0) {
        this.hsArch = a;
        break;
      }
    }
  ...
  plotDecklistsMatchups(hsArch) {
    // load deck matchups in right sidebar
    let f = this.f;
    let wr = hsArch.wr;
    let top = 3;
    let topArch = [];
    ...
    if (wr > 0) {
      ...
      if (idx == -1) {
        hsArch.wr = 0;
        this.plotDecklistsMatchups(hsArch);
        return;
      }
  ...
  plotDustWr() {
    if (this.archetypes[this.f].length == 0) {
      return;
    }
    ...
    for (let a of this.archetypes[this.f]) {
      ...
      for (let dl of a.decklists) {
        ...
        traces.push({
          x: [a.wr],
          y: [dl.dust],
          text: `<b>${dl.name}</b><br>Winrate: ${(a.wr * 100).toFixed(
            2
          )}%<br>Dust Cost: ${dl.dust}`,
          hoverinfo: "text",
          name: dl.name,
          mode: "markers",
          type: "scatter",
    ...
    document
      ...
      .on("plotly_click", clickHandler.bind(this));
  } // plot dust vs wr
