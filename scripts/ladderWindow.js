class LadderWindow {
  constructor(callback) {
    ...
    this.classDeckOptions = document.querySelector(
      "#ladderWindow .content-header .classDeckOptions"
    );
    // this.nrGamesBtn = document.querySelector('#ladderWindow .content-header #nrGames')
    this.nrGamesBtn = document.querySelector(
      "#ladderWindow .content-header #showNumbers"
    );
    ...
    this.rankFolder = document.querySelector(
      "#ladderWindow .content-header #rankBtn"
    );
    ...
    // removed  (rank 20 - Legend) text from below text
    this.overlayText.number = `
        This table displays the class/ deck frequencies over ladder ranks. You can vary the rank brackets in the header.<br><br>
        In <span class='optionBtn'>Decks</span> mode decks with 3% or lower frequencies have been merged with the 'Other' decks of that class.<br><br>
  ...
  setupUI() {
    ...
    document.querySelector(
      "#ladderWindow .content-header .graphOptions #line"
    ).style.display = disp;
    //document.querySelector('#ladderWindow .content-header .graphOptions #number').style.display = disp
    document.querySelector(
      "#ladderWindow .content-header .graphOptions #timeline"
    ).style.display = disp;
  ...
  checkLoadData(callback) {
    ...
    if (!this.data[this.f].fullyLoaded) {
      let callback2 = function () {
        app.ui.ladderWindow.checkLoadData(callback);
      };
      if (back) {
        return this.loadData(this.f, callback2);
      } else {
        return false;
      }
    }
    ...
    if (
      this.plotType == "map" &&
      !app.ui.tableWindow.data[this.f].fullyLoaded
    ) {
      let callback2 = function () {
        app.ui.ladderWindow.checkLoadData(callback);
      };
      if (back) {
        return app.ui.tableWindow.loadData(this.f, callback2);
      } else {
        return false;
      }
    }
    ...
    if (
      this.plotType == "timeline" &&
      !this.history[this.f].fullyLoaded &&
      PREMIUM
    ) {
      let callback2 = function () {
        app.ui.ladderWindow.checkLoadData(callback);
      ...
      if (back) {
        return this.loadHistoryData(this.f, callback2);
      } else {
        return false;
      }
    }
  ...
  loadData(hsFormat, callback) {
    ...
    let reader = function (DATA) {
      this.readData(DATA, hsFormat, callback);
    };

    ref.on("value", reader.bind(this), (e) =>
      console.log("Could not load Ladder Data", e)
    );
  }
  ...
  loadHistoryData(hsFormat, callback) {
    if (PREMIUM) {
      ...
      let reader_h = function (DATA) {
        this.readHistoryData(DATA, hsFormat, callback);
      };
      ref_h.on("value", reader_h.bind(this), (e) =>
        console.log("Could not load history data", e)
      );
    }
  }
  ...
  readData(DATA, hsFormat, callback) {
    ...
    //console.log(ladderData.last2Weeks);
    console.log({ ladderData: ladderData });
    ladderData = modLadderData(ladderData, this.hsTimes);
    for (let t of this.hsTimes) {
      this.data[hsFormat][t] = new Ladder(ladderData[t], hsFormat, t, this);
    }
    this.fullyLoaded = true;
    this.data[hsFormat].fullyLoaded = true;
    console.log(
      "ladder loaded: " + (performance.now() - t0).toFixed(2) + " ms"
    );
  ...
  plot() {
    switch (this.plotType) {
      ...
      case "pie":
        ...
        if (!PREMIUM) {
          this.classDeckOptions.style.display = "flex";
        }
        break;
    ...
    if (this.plotType == "timeline") {
      this.history[this.f].plot();
      return;
    }
  ...
  annotate() {
    if (this.plotType == "pie" || this.plotType == "number") {
      return;
    }
    if (this.annotated) {
      if (this.plotType == "timeline") {
        this.history[this.f].annotate(false);
      } else {
        this.data[this.f][this.t].annotate(false);
      }
      this.nrGamesBtn.classList.remove("highlighted");
    } else {
      if (this.plotType == "timeline") {
        this.history[this.f].annotate(true);
      } else {
        this.data[this.f][this.t].annotate(true);
      }
  ...
  buttonTrigger(e) {
    ...
    if (this.plotType == "zoom" && this.mode != "classes") {
      this.plotType = "bar";
    }
  ...
  renderOptions() {
    for (let btn of this.optionButtons) {
      ...
      if (btn.id == this.plotType) {
        btn.classList.add("highlighted");
      }
      if (btn.id == "nrGames" && this.annotated) {
        btn.classList.add("highlighted");
      }
    }
    document.querySelector("#ladderWindow #formatBtn").innerHTML = MOBILE
      ? btnIdToText_m[this.f]
      : btnIdToText[this.f];
    document.querySelector("#ladderWindow #timeBtn").innerHTML = MOBILE
      ? btnIdToText_m[this.t]
      : btnIdToText[this.t];
    document.querySelector("#ladderWindow #rankBtn").innerHTML = MOBILE
  ...
  setGraphTitle() {
    ...
    var time =
      ["lastDay", "last6Hours", "last12Hours"].indexOf(this.t) != -1
        ? "Hours"
        : "Days";
    var rank = btnIdToText[this.r];
    let data = this.data[this.f][this.t];
    let nrGames =
      this.plotType != "pie" ? data.totGames : data.totGamesBrackets[this.r];
    ...
    switch (this.plotType) {
      ...
      case "zoom":
        this.graphTitle.innerHTML =
          this.zoomClass + " Deck Frequency vs Ranks" + games;
        this.graphLabel.innerHTML = "Ranks >";
        break;
      case "line":
  ...
  addLegendItem(archName) {
    ...
    legendDiv.style =
      "background-color:" + colors.color + "; color:" + colors.fontColor;
    legendDiv.id = archName;
    legendDiv.innerHTML = archName;
    legendDiv.onclick = function (e) {
      if (app.ui.decksWindow == null) {
        return;
      }
      app.path.hsFormat = this.f;
  ...
  clearChartFooter() {
    while (this.chartFooter.firstChild) {
      this.chartFooter.removeChild(this.chartFooter.firstChild);
    }
  }
